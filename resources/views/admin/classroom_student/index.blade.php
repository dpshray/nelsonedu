                        ...
                        <div class="">
                            <nav>
                                <ol class="breadcrumb mb-0">
                                    ...
                                    <li class="breadcrumb-item active" aria-current="page">List of Classroom Enrollable Students </li>
                                </ol>
                            </nav>
                        </div>
                    </div>
                            ...
                            <div class="card-body">
                                <div class="table-responsive">
                                    <table class="table text-nowrap table-bordered">
                                        ...
                                        <tbody>
                                            @foreach($classroomStudents as $index => $cstudent)
                                            <tr>
                                                <th scope="row">
                                                ...
                                                <td>{{ $cstudent->student->email }}</td>
                                                <td>{{ $cstudent->status == 1 ? 'Approved' : 'Pending' }}</td>
                                                <td>{{ $cstudent->payment_status }}</td>
                                                <td>{{ $cstudent->remarks }}</td>

                                                <td>
                                                    <div class="hstack gap-2 flex-wrap">
                                                        <!-- <form action="{{ route('admin.classroom.student.update', $cstudent['id']) }}" method="POST">
                                                            @csrf
                                                            @method('PUT')
                                                            <a href="javascript:;" onclick="enrollConfirmation(event)" class="text-info fs-14 lh-1" data-bs-toggle="tooltip" data-bs-placement="top" title="Approves">
                                                                <i class="ri-restart-line"></i>
                                                            </a>
                                                        </form> -->
                                                        <button type="button" class="btn btn-primary openModal" data-id="{{ $cstudent['id'] }}">
                                                            <i class="ri-restart-line"></i>
                                                        </button>
                                                    </div>
                                                </td>
                                    ...
                                    {{ $classroomStudents->links('pagination::bootstrap-5') }}
        ...
        <script>
            $(document).ready(function() {
                $(".openModal").click(function() {
                    let id = $(this).data('id');
                    let actionUrl = "{{ route('admin.classroom.student.update', '__id__') }}".replace('__id__', id);
